...
namespace Cards
{
    ...
    {
        _delete_deck();
        if (_numdecks > max_decks)
            throw CE("Deck: too many decks (max 10)");
    ...
    void Deck::shuffle_deck()
    {
        ...
        for (uint16 i = 0; i < ncards; ++i)
        {
            ...
            if (r != i)
                std::swap(_deck[r], _deck[i]);
        }
    }

    const Card &Deck::get_card(uint16 index) const
    {
        if (index >= _remaining_count)
            throw CE("Deck: card out of range");
        return _deck[index];
    }
    ...
    const Card &Deck::deal_card()
    {
        if (_remaining_count == 0)
            throw CE("Deck: cannot deal from empty deck");
    ...
    const char *card_str(const Card &c)
    {
        ...
        if (c.rank == 0)
            outstr_rank = facecards[0]; // rank 0 is A
        else if (c.rank > 8)
            outstr_rank = facecards[c.rank - 8]; // T J Q K
        else
            outstr_rank = c.rank + '1'; // numeric cards
